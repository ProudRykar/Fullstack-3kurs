...
class ErrorCode(Enum):
    ...
    IMAGE_UPLOAD_ERROR = ErrorMeta(
        "image-upload-error", "Ошибка загрузки изображения", 400
    )
    IMAGE_DELETION_ERROR = ErrorMeta(
        "image-deletion-error", "Ошибка удаления изображения", 400
    )
    UNSUPPORTED_MEDIA_TYPE = ErrorMeta(
        "unsupported-media-type", "Не поддерживаемый формат изображения", 415
    )

    # Аутентификация (401)
    AUTHENTICATION_ERROR = ErrorMeta(
        "authentication-error", "Ошибка аутентификации", 401
    )
    ...
    # Не найдено (404)
    NOT_FOUND = ErrorMeta("not-found", "Не найдено", 404)
...
class ProblemFactory:
    ...
    def build_from_meta(
        self, meta: ErrorMeta, detail: str, **extra: Any
    ) -> dict[str, Any]:
